...
```
                   ...
                   └──────────────────────┬───────────────────────┘
                                          ...
                                          ▼
                   ┌──────────────────────────────────────────────┐
                   │ ApiKeyValidationFilter (Order: HIGHEST + 10) │
                   │  - Extracts X-API-Key                        │
...
| `Ordered.HIGHEST_PRECEDENCE + 10` (`-2147483638`) | `ApiKeyValidationFilter` | Global Filter | Enforces zero-trust API key authentication before route processing. Strips the credential header to prevent upstream credential leakage. |
